...
def capture_agent_response(agent, message: str, max_attempts: int = 3) -> str:
    ...
    for attempt in range(max_attempts):
        try:
            # Método 1: Capturar stdout (más confiable)
            ...
            # Prioridad 1: Si print_response devuelve contenido válido
            if result and str(result).strip() and str(result) != "None":
                return str(result).strip()
            
            # Prioridad 2: Si stdout tiene contenido
            if captured_stdout:
                # Filtrar líneas de debug y metadata
                lines = captured_stdout.split('\n')
                content_lines = []
                for line in lines:
                    line = line.strip()
                    # Excluir líneas de debug, metadata y decoraciones
                    if (line and 
            ...
            # Método 2: Intentar con run() si está disponible
            try:
                result2 = agent.run(message, stream=False)
                if result2 and str(result2).strip() and str(result2) != "None":
